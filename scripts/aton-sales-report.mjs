...
if (!/^\\d{2}\\/\\d{2}\\/\\d{4}$/.test(start||'') || !/^\\d{2}\\/\\d{2}\\/\\d{4}$/.test(end||'')) throw new Error('Datas inválidas');

const get=(o,names)=>{for(const n of names)if(o&&o[n]!==undefined&&o[n]!==null&&o[n]!=='')return o[n];};
const txt=v=>typeof v==='string'?v.trim():'';
const num=v=>{if(typeof v==='number')return Number.isFinite(v)?v:0;if(typeof v!=='string')return 0;const s=v.trim().replace(/R\\$\\s?/g,'');const x=s.includes(',')?s.replace(/\\./g,'').replace(',','.'):s;const n=Number(x);return Number.isFinite(n)?n:0;};
...
const md=['# Vendas por vendedor','','Período: '+start+' a '+end,'','| # | Vendedor | Pedidos | Faturamento | Ticket médio |','|---:|---|---:|---:|---:|',...result.map(r=>'| '+r.posicao+' | '+r.vendedor.replace(/\\|/g,'/')+' | '+r.pedidos+' | '+brl.format(r.faturamento)+' | '+brl.format(r.ticket_medio)+' |')].join('\\n');
