...
async function authorized(req) {
  ...
  try {
    const { payload } = await jwtVerify(token, await jwks(), { issuer: ISSUER, audience: AUDIENCE });
    return payload.sub === SUBJECT && payload.project === 'aton-innovex-readonly' && payload.environment === 'production';
  } catch { return false; }
}
...
export default async function handler(req, res) {
  ...
  for (const name of allowed) {
    if (q[name] !== undefined && q[name] !== null) body[name] = q[name];
  }
  ...
  try {
    ...
    let data;
    try { data = JSON.parse(text); } catch { data = { data: text.slice(0, 12000) }; }
    return reply(res, upstream.status, data);
  } catch (e) {
    return reply(res, 502, { error: 'upstream_unavailable', message: e?.message || null });
  }
}
